...
export default async function SubmissionsPage({ searchParams }: { searchParams: Promise<{ productId?: string; status?: string; q?: string; view?: string }> }) {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4 rounded-3xl bg-[#172b54] px-6 py-7 text-white shadow-lg sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-sky-200">FOT operations</p><h1 className="mt-1 text-2xl font-bold">Campaign submissions</h1><p className="mt-1 text-sm text-white/65">Manage every campaign grouped under its Sales Order.</p></div><a href={exportHref} className="inline-flex shrink-0 items-center justify-center rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-[#172b54] transition hover:bg-sky-50">Export CSV</a></div>
