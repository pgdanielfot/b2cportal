...
function StoryPost({ instagram, media, action }: { instagram: boolean; media?: Media; action: string }) {
  return <div className="mx-auto w-[260px] overflow-hidden rounded-[1.4rem] border-[5px] border-[#111b30] bg-[#111b30] shadow-[0_16px_30px_rgba(20,35,60,0.28)]">
    <div className="relative aspect-[9/16] overflow-hidden bg-[#dce6f5]">
      ...
      <div className="absolute inset-x-3 top-6 flex items-center gap-2 text-white"><Avatar instagram={instagram} /><div><p className="text-[10px] font-bold drop-shadow">PropertyGuru / iProperty</p><p className="text-[9px] opacity-85">Sponsored</p></div><span className="ml-auto text-sm">•••</span></div>
...
function Avatar({ instagram }: { instagram: boolean }) { return <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[10px] font-bold text-white ${instagram ? "bg-gradient-to-br from-fuchsia-500 via-rose-500 to-amber-400" : "bg-[#2169df]"}`}>PG</span>; }
